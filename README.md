...
- `data.rda` single synthetic dataset used in guided example, generated by `sim-guided-exmaple-data.R` and used in `analyze-guided-example.R`
...
- `do_imp_function.R` code for function `do_imp()`, which is used in `analyze-guided-example.R`
