...
# In this third setting, we have only 3 dose levels and the first dose is the MTD
...
d <- c(0.25, 0.5, 0.55) # true toxicity per dose
...
save(
  boins_4,
  suff_safe_4,
  file = here::here("data", "escalate-sim-res-4.rda")
)
  ...
  map(
    ...
    ~ map(
      ...
      function(a) tibble( 
        ...
        rowwise() |> 
        # Need to account for possible NAs in suff_safe (i.e. MTD is dose 1 or 2 so there aren't 3 sufficiently safe doses)
        mutate(
...
save(
  ...
  dose_expand_summ_4,
  dose_rank_4,
  file = here::here("data", "expand-sim-res-4.rda")
)
  ...
  map(
    ...
    function(p_index){
      map(
        1:S,
        function(S_index) {
          if(any(!is.na(efficacy_tab_4[[p_index]][[S_index]]$biomark_grp))) { # Check for NA
            efficacy_tab_4[[p_index]][[S_index]] |> 
              # Need to add in the information for treated patients from the dose expansion stage
              bind_rows(
                dose_expand_tab_4[[p_index]][[S_index]] |> 
                  right_join(dose_rank_4[[p_index]][[S_index]] |> 
...
save(
  efficacy_tab_4,
  efficacy_summ_4,
  file = here::here("data", "efficacy-sim-res-4.rda")
)
