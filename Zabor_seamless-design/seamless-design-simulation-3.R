...
# In this third setting, we have only 3 dose levels and the second dose is the MTD
...
d <- c(0.15, 0.25, 0.5) # true toxicity per dose
...
save(
  boins_3,
  suff_safe_3,
  file = here::here("data", "escalate-sim-res-3.rda")
)
...
save(
  ...
  dose_expand_summ_3,
  dose_rank_3,
  file = here::here("data", "expand-sim-res-3.rda")
)
  ...
  map(
    ...
    function(p_index){
      map(
        1:S,
        function(S_index) {
          if(any(!is.na(efficacy_tab_3[[p_index]][[S_index]]$biomark_grp))) { # Check for NA
            efficacy_tab_3[[p_index]][[S_index]] |> 
              # Need to add in the information for treated patients from the dose expansion stage
              bind_rows(
                dose_expand_tab_3[[p_index]][[S_index]] |> 
                  right_join(dose_rank_3[[p_index]][[S_index]] |> 
...
save(
  efficacy_tab_3,
  efficacy_summ_3,
  file = here::here("data", "efficacy-sim-res-3.rda")
)
